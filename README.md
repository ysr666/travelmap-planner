...
旅图可以选择接入 Supabase，用于账号登录后的旅行对象同步和恢复。它不是实时协作：不会做字段级多人协作合并，也不会静默覆盖同一对象的双边修改。只要用户已登录且自动同步开启，本机成功写入的旅行、行程点、票据和备注会进入同步队列；用户也可以在设置页点击“立即同步”。
...
- 自动云端同步：在此设备数据变化后自动补传对象更新；不同对象可合并，同一对象双边修改时会提示选择同步方向。
...
- Supabase 云端同步不是实时协作，不做字段级合并；同一对象可能双向修改时需要用户手动选择。

## 项目状态

当前为 offline-first PWA：本机先落盘，登录后自动同步账号数据。项目可选接入 Supabase 对象同步、TripMap provider proxy 路线服务和浏览器可见的 Google Maps JS 渲染 key；server-only Google Routes / OpenRouteService / future AI keys 不应进入前端 bundle 或用户设置页。
