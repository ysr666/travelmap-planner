...
**当前：** 配置路线服务后，“今日”会优先读取与当前地点、顺序、交通方式和 Provider 完全匹配的道路路线缓存；精确缓存不存在时，会通过 Provider Proxy 自动生成并缓存当前日道路路线。用户主动授权当前位置后，当前位置到首站会另行请求一次临时道路路线，不写入旅行记录或持久路线缓存。手动批量生成和顺序建议仍保留；不提供语音导航、turn-by-turn、实时交通、实时公交班次或离线路线。地点搜索与补坐标由独立 Place 操作处理。路线请求失败时只显示低强调的虚线地点顺序，不显示道路方向箭头，也不能把它称为真实路线。

“今日”底图在已配置受限 Google Maps JavaScript 渲染 key 时优先使用 Google 地图；若脚本未在启动预算内可用或初始化失败，则自动回退到 MapLibre / OpenFreeMap。底图选择只影响道路、地名和地图交互呈现，不改变路线来源：道路 polyline 仍只来自 Provider Proxy 或精确路线缓存。稳定视觉 E2E 固定使用 MapLibre fixture，另有显式在线底图取证模式验证真实地图渲染。

稳定视觉 E2E 的固定底图和固定 polyline 只用于布局回归，不能作为“真实地图”或“真实 Provider 路线”的证据。真实视觉取证必须同时启用在线底图，并通过 `UI_V3_REAL_ROUTE_FIXTURE` 注入本次由 OpenRouteService 实际返回、已脱敏归一化的路线坐标；测试会拒绝未知来源或结构无效的路线文件，并把截图写入独立的 `ui-v3-real-map` 目录，避免与 Golden 混用。
...
当前生成道路路线时，旅图把相邻行程点坐标发送给 TripMap 路线服务及其后端 Provider，不发送地点备注、票据或完整账号数据。用户主动请求当前位置后，临时首站路线只发送当前位置、首站坐标、交通方式和归一化目标 ID；不发送票据、备注或其他日程内容。路线顺序建议只把坐标发送给真实 Provider；标题和 ID 仅用于 TripMap proxy 归一化与确认展示。
...
下次打开同一 Trip / Day 时，如果行程点坐标、顺序、交通模式和 provider 版本没有变化，地图会自动显示精确缓存路线。即使路线服务暂不可用，也可以查看已有缓存路线；路线缓存 signature 不包含 API key、环境变量来源或 localStorage key 值。当前位置到首站的路线不会写入 `TripMapRouteCacheDB`，位置变化后必须重新取得。

如果地点坐标、排序、交通模式或路线算法版本变化，旧缓存会失效并删除；“今日”会尝试为新状态生成一次道路路线，失败时才回到虚线地点顺序。清理路线缓存后，当前地图页会收到 `tripmap:route-cache-changed` 事件并刷新显示；同一页面会话不会对同一状态反复请求，重新进入“今日”后才会再次自动生成。
...
道路路线按相邻地点分段生成。只要当日存在无法验证的路段，地图就把整组线降级为虚线地点顺序，避免成功段与点间连接混在一起后被误认成完整道路路线。
...
无论哪种失败，已缓存行程、marker、bottom sheet 和虚线地点顺序都应继续可用。
