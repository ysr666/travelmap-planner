...
async function openItemMore(page: Page) {
  const details = page.getByTestId('item-detail-more')
...
test('日程来源打开行程点详情并返回日程', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '打开行程点 Hotel Metropolitan Tokyo 入住' }).click()
  ...
  await expect(page.getByRole('link', { name: '开始导航' })).toBeVisible()
  await expect(page.getByRole('button', { name: '添加票据' })).toBeVisible()
  await expect(page.getByTestId('item-detail-tickets')).toContainText('关联票据')
  await expect(page.getByTestId('item-detail-tickets')).toContainText('暂无关联票据')
  await expect(page.getByTestId('item-detail-more')).not.toHaveAttribute('open', '')
  await openItemMore(page)
  await expect(page.getByTestId('item-detail-more').getByRole('link', { name: 'Apple 地图' })).toBeVisible()
  await expect(page.getByTestId('item-detail-more').getByRole('link', { name: 'Google 地图' })).toBeVisible()
...
test('日程来源详情可按当天顺序切换上一项和下一项', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/view=schedule/)
  await openItemMore(page)
  await expect(page.getByTestId('item-field-previous-stop')).toBeDisabled()
  await expect(page.getByTestId('item-field-next-stop')).toBeEnabled()

  await page.getByTestId('item-field-next-stop').click()
  await expect(page).toHaveURL(/#\/item\?/)
  await expect(page).toHaveURL(/view=schedule/)
  await expect(page.getByTestId('item-detail-core')).toContainText('明治神宫散步')
  await openItemMore(page)
  await expect(page.getByTestId('item-field-previous-stop')).toContainText('Hotel Metropolitan Tokyo 入住')
  await expect(page.getByTestId('item-field-next-stop')).toContainText('Shibuya Sky 夜景')
  await expect(page.getByTestId('item-detail-more').getByRole('link', { name: 'Apple 地图' })).toBeVisible()
  await expect(page.getByTestId('item-detail-more').getByRole('link', { name: 'Google 地图' })).toBeVisible()
  await expect(page.getByTestId('item-field-previous-stop')).toBeEnabled()

  await page.getByTestId('item-field-previous-stop').click()
  ...
  await openItemMore(page)
  await expect(page.getByTestId('item-field-next-stop')).toBeDisabled()
  await expectNoHorizontalOverflow(page)
})
...
test('无坐标但有地址的行程点仍可轻量导航', async ({ page }) => {
  ...
  await expect(page.getByTestId('item-detail-page')).toBeVisible()
  await expect(page.getByTestId('item-detail-core')).toContainText('1-1 Yoyogikamizonocho')
  await expect(page.getByRole('link', { name: '开始导航' })).toBeVisible()
  await openItemMore(page)
  await expect(page.getByTestId('item-detail-more').getByRole('link', { name: 'Apple 地图' })).toBeVisible()
  await expect(page.getByTestId('item-detail-more').getByRole('link', { name: 'Google 地图' })).toBeVisible()
...
test('票据区显示现场卡片、预览和查看全部入口', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '打开票据' })).toBeVisible()
  await page.getByRole('button', { name: '打开票据' }).click()
  ...
  await expect(page.getByTestId('item-ticket-view-all')).toContainText('查看全部')
  ...
  expect(hash).toContain(`itemId=${firstItemId}`)
  await expectNoHorizontalOverflow(page)
})
