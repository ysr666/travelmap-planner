...
- Supabase 云端备份：登录后手动上传旅行快照，并恢复为新的本地旅行
...
旅图可以选择接入 Supabase，用于账号登录后的云端快照备份和恢复。它不是实时同步：不会自动后台上传，不会合并多设备冲突，也不会多人协作。
...
- external 模式只保存外部链接。
- 应用不会自动把数据上传到服务器；只有用户主动启用并点击云端备份时，才会上传到 Supabase。
