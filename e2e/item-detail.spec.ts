...
test('日程来源打开行程点详情并返回日程', async ({ page }) => {
  ...
  await expect(page.getByTestId('item-field-action-deck')).toContainText('现场行动')
  await expect(page.getByTestId('item-field-action-deck')).toContainText('第 1/3 项')
  await expect(page.getByTestId('item-detail-core')).toContainText('15:00')
  await expect(page.getByTestId('item-field-place-action').getByRole('link', { name: 'Apple 地图' })).toBeVisible()
  await expect(page.getByTestId('item-field-place-action').getByRole('link', { name: 'Google 地图' })).toBeVisible()
...
test('日程来源详情可按当天顺序切换上一项和下一项', async ({ page }) => {
  ...
  await expect(page.getByTestId('item-field-action-deck')).toContainText('第 2/3 项')
  ...
  await expect(page.getByTestId('item-field-route-action').getByRole('link', { name: 'Apple 地图' })).toBeVisible()
  await expect(page.getByTestId('item-field-route-action').getByRole('link', { name: 'Google 地图' })).toBeVisible()
...
test('无坐标行程点显示轻量导航不可用状态', async ({ page }) => {
  ...
  await expect(page.getByTestId('item-field-action-deck')).toContainText('待补坐标')
  await expect(page.getByTestId('item-field-route-action').getByRole('link', { name: /Apple 地图|Google 地图/ })).toHaveCount(0)
  await expect(page.getByTestId('item-field-place-action').getByRole('link', { name: /Apple 地图|Google 地图/ })).toHaveCount(0)
...
test('票据区显示现场卡片、预览和查看全部入口', async ({ page }) => {
  ...
  await expect(page.getByTestId('item-field-action-deck')).toContainText('4 张票据')
  await page.getByTestId('item-field-action-deck').getByRole('button', { name: /打开票据/ }).click()
