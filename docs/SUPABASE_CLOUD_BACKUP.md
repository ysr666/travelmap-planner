...
本功能不做实时同步、多设备冲突合并、多人协作或云端编辑。应用支持用户手动上传云端快照，也支持用户在设置页主动开启“自动云端备份”，在本地数据变化后延迟上传新的旅行快照。
...
- 自动云端备份只是可选的快照上传，不会自动恢复、不会覆盖本地，也不会合并多设备修改。
...
设置页可以开启“自动云端备份”。开启后，旅图会在本地 Trip / Day / Item / Ticket 数据成功变更后，把对应旅行标记为待备份，并在用户已登录、Supabase 已配置、浏览器在线时延迟上传新的云端快照。
...
- 不会覆盖本地 IndexedDB。
- 不会合并多设备冲突。
- 删除本地旅行不会自动删除云端备份；云端删除仍需要用户手动确认。
- 上传失败不会阻止本地编辑，只会保留待备份状态并显示“云端备份失败，可稍后重试”。
...
不是。它是快照备份。用户可以手动上传，也可以主动开启自动云端备份，让应用在本地变更后延迟上传新快照；但它不会实时同步、不会自动恢复，也不会自动合并多设备修改。
