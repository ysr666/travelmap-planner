...
export function ListRow({ icon, title, detail, meta, onClick }: ListRowProps) {
  const content = (
    <>
      ...
      <span className="min-w-0 flex-1">
        ...
        {detail ? (
          <span className="block truncate text-sm tm-muted">{detail}</span>
        ) : null}
      </span>
  ...
  if (onClick) {
    return (
      <button
        className="flex w-full items-center gap-3 rounded-xl px-2 py-3 text-left transition active:bg-slate-50/80 dark:active:bg-slate-800/50 tm-focus"
        onClick={onClick}
        type="button"
      >
        {content}
