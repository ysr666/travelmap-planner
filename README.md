...
- Supabase 云端快照：登录后手动上传本地快照，可选开启自动云端快照备份，并恢复为新的本地旅行副本
...
- Root directory：`/`
- Environment variables：如果启用云端快照备份，需要配置 `VITE_SUPABASE_URL` 和 `VITE_SUPABASE_ANON_KEY`；如果启用道路路线，需要配置 `VITE_ROUTING_PROVIDER` 和 `VITE_OPENROUTESERVICE_API_KEY`

项目使用 hash 路由，静态部署时不依赖服务器重写规则。

## ☁️ Supabase 云端快照

旅图可以选择接入 Supabase，用于账号登录后的云端快照备份和恢复。它不是实时同步：不会合并多设备冲突，也不会多人协作。你可以手动上传本地快照，也可以在设置页主动开启“自动云端快照备份”，让旅图在本机数据变化后延迟上传新的云端快照。

登录且在线时，旅图会做一次轻量启动检查：比较本地旅行版本信号与最新云端快照 metadata。如果发现云端快照较新、本地版本较新或可能冲突，只显示非阻塞中文提示，由用户手动选择查看云端快照、上传本地快照或恢复为新旅行副本。
...
- 云端快照：上传旅行结构化数据和 copy 模式票据附件到 Supabase，适合换设备或清空浏览器后的恢复。每次上传都会创建新的云端快照，不会删除旧快照。
- 自动云端快照备份：只是在本机数据变化后自动上传新的云端快照，不会自动恢复、不会覆盖本地数据，也不会合并多设备修改。
- 启动云端快照检查：只比较 metadata 并提示用户，不会自动恢复、覆盖、合并或删除任何数据。
- 恢复云端快照会创建一个新的本地旅行副本，不覆盖现有 IndexedDB 数据，也不会删除云端快照。恢复副本会带来源标识。
- 第一版未做端到端加密，护照、签证、银行卡等高度敏感文件请谨慎上传。

Supabase 建表、RLS 和 Storage policy 见 [Supabase 云端快照配置](docs/SUPABASE_CLOUD_BACKUP.md)。

## 🗺️ 道路路线 Polyline

地图默认使用直线连接当天地点。可选配置 OpenRouteService 后，在地图页手动点击“生成道路路线”，旅图会按相邻地点请求道路路线 polyline。失败、超时、额度不足或交通模式不支持时，会回退显示直线。
...
注意：`VITE_OPENROUTESERVICE_API_KEY` 会进入前端 bundle。个人部署可以使用；公开部署不建议把 provider key 打进前端，未来公开服务应使用后端代理。本阶段不做后端代理。

也可以在设置页把 OpenRouteService key 保存到当前浏览器本机。这个 key 不进入 IndexedDB、zip 备份、Supabase 云备份或 AI 行程包。

道路路线生成成功后会保存为本地路线缓存，只存在当前浏览器的独立 `TripMapRouteCacheDB` 中，不进入 zip 备份、Supabase 云备份或 AI 行程包。下次打开同一旅行和同一天时，如果地点坐标、顺序和交通方式没有变化，地图会自动显示“本地缓存路线”；即使当前没有配置 OpenRouteService key，也可以查看已有缓存，但不能重新生成。修改地点坐标、顺序或交通方式后，旧路线缓存会失效并删除。设置页可以查看缓存大小、设置上限并清理缓存。

公交段会使用驾车道路路线做近似，不包含公交站点、班次、换乘和实时交通；火车、公共交通和飞机段仍使用直线 fallback。

道路路线不是实时导航，不包含实时交通。生成路线时会把地点坐标发送给第三方路线服务。详细说明见 [地图道路路线 Polyline](docs/ROUTING.md)。
...
- external 模式只保存外部链接。
- 应用不会默认把数据上传到服务器；只有用户手动上传本地快照，或主动开启“自动云端快照备份”后，才会上传到 Supabase。
...
- Supabase 云端快照不是实时同步，不做自动恢复、自动覆盖或自动冲突合并。

## 项目状态

当前为个人使用型 PWA，优先 local-first。项目可选接入 Supabase 云端快照备份、OpenRouteService 道路路线和用户自配的 Google Maps 能力，但不包含自建后端、实时云同步、Mapbox token 或任何内置收费服务。
