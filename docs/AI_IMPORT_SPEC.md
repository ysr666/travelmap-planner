...
本格式是外部 AI、旧工具和批量迁移使用的兼容入口，不是旅图未来的主要 AI 体验。主要入口是应用内全局 AI Action Gateway：用户直接表达目标，系统在线获取所需事实、生成短预览，并在一次确认后执行可逆写入。

当前 `trip-plan.json` / `trip-plan.zip` 导入只读取用户选择的文件，不上传服务器；AI Draft 仍可通过 mock 或 Provider Proxy 生成和修复草稿。导入和草稿应用都必须经过预览及最终确认。后续在线导入任务会复用同一 schema 校验、动作注册表和服务端 revision 防过期机制。
...
`trip-plan.zip` 不是旅图完整 zip 归档。完整 zip 归档使用 `manifest.json + data/*.json + files/*` 结构，请在设置页使用“导入归档”入口。
