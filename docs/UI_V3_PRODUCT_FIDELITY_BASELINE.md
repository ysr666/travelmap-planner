...
状态：**P0 Baseline Current；P1-P6 已完成本地验证，剩余精修与发布差异待 P7-P8 关闭**
...
| G-04 | P1 | 页面是无框分组、行、分隔线与少量独立记录 | Today 与时间轴使用无框分组和分隔线；资料仅为独立文档记录保留边界 | P6/P7 | implemented |
| G-05 | P1 | 顶栏、标题、日期条、内容和底栏在四屏具有统一节奏 | 四屏已按 `390x844` 同状态构图并完成首轮视觉检查 | P6/P7 | implemented |
| G-06 | P2 | 所有图像在加载、失败、离线和切换尺寸时保持稳定比例 | 共享媒体组件与固定 Hero/缩略图比例已接入五个页面，异常状态待 P7 扩展验证 | P1/P7 | implemented |
...
| U-07 | P2 | 三个对象行保持一致的 72-96px 节奏、细边框和清晰元数据层级 | 三类对象使用统一 72px 行、细分隔线和三级元数据 | P6/P7 | implemented |
...
| A-06 | P2 | Hero 照片不压暗到不可检查，文字叠层满足对比度且保留主体焦点 | 照片保持原亮度并使用独立文字区和焦点裁切，最终对比度复核待 P7 | P1/P7 | implemented |
...
| T-06 | P2 | 缩略图为固定 `4:3`，长标题与英文地址仍保留操作列 | 固定媒体尺寸、两行标题和独立 44px 操作列已实现，最终长文本矩阵待 P7/P8 | P1/P7 | implemented |
...
| ID | 级别 | Selected Target | 当前实现 | 责任阶段 | 状态 |
| --- | --- | --- | --- | --- | --- |
| D-01 | P0 | 资料首页是编辑式单列列表，左侧真实票面/PDF 首页缩略图 | 可用票面/PDF/受控媒体优先展示；无票面但可识别品牌的对象显示品牌，最后才降级文件类型 | P1/P6 | implemented |
...
| D-04 | P1 | 门票、保险、火车票、酒店分别具有真实而不同的视觉预览 | 门票/酒店/铁路使用真实媒体，航司/保险使用受控品牌；缺失时稳定降级 | P1/P2/P6 | implemented |
...
| I-04 | P2 | 所有媒体支持 loading/error/expired/offline/reduced-data，且不改变布局尺寸 | loading/error/expired 与固定尺寸已实现；offline/reduced-data 验收待 P7 | P1/P7 | implemented |
