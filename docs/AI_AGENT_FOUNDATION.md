...
3. 不发送 ticket IDs、票据文件名/内容/blob、完整坐标、cloud token/status、route cache、provider key、URL 或完整本地 DB。
4. 用户通过第一道 ConfirmDialog 确认后，才调用 `ai_trip_edit_plan`。
5. 返回内容必须经过 JSON extraction 和 `validateAiTripEditPatchPlan`。验证器是 source of truth；不信任 AI 输出的 affected IDs/counts/source/metadata。
6. UI 只显示 diff preview；preview 出现不代表已修改旅行。影响天数和项目数由本地根据已验证 operation 推导。
7. 用户通过第二道 ConfirmDialog 确认后，才重新读取当前 IndexedDB 状态、检查 preview-time baseline 是否仍匹配，并在本地 transaction 中应用 patch。
...
- 多轮聊天、对话记忆、新的 `ai_trip_edit` operation、自主 web search 或 provider-internal tool-calling。
...
搜索 provider 也只能配置在 server-side env：

- `TRIPMAP_SEARCH_PROVIDER=disabled|mock|tavily`。
