...
- built-dist E2E 覆盖 `v1 → v2 → v3` 连续 Service Worker 修订；每次更新都保持 waiting，直到一次用户确认后所有标签收敛。
- 连续升级之间的离线 IndexedDB 修改在后续激活和标签重载后保持不变。
- Chromium origin quota 压力验证确认按需资源仍返回完整网络响应，不写入残缺缓存；恢复存储空间后可完整缓存并离线复用。
- PWA 四项测试连续 5 轮共 20/20 通过，完整串行 E2E 共 154 项通过。

第六轮已完成：

- 账号旅行在 Chromium 离线状态下编辑时只更新 IndexedDB、自动快照 dirty 状态和对象 outbox，账号 fixture 保持不变。
- 恢复在线后由现有 `online` 事件自动续传；同一快照原地覆盖，trip/item 对象各保持一条，outbox 清空且状态收敛为 `synced`。
- 聚焦恢复用例连续 10 轮通过，云端同步文件 13/13 通过，完整串行 E2E 共 155 项通过。

第七轮已完成：

- 固定两个曾部署的 `main` 提交及其 Git tree/package lock，现场生成真实历史 Vite/PWA 产物，不依赖可变远端部署下载。
- 同一 origin 连续执行 `4c8f60ec → 4c748935 → 当前候选`；每次确认前 worker 保持 waiting，确认后双标签收敛。
- 第一版创建的真实旅行和中间版本离线修改经过两次升级仍可打开，最终只保留一个当前 precache。
- 历史迁移聚焦 5/5、完整 PWA 5/5、完整串行 E2E 156/156 通过。
...
3. 建立真实设备首屏加载和交互时间基线，并补生产缓存头与部署 SHA 诊断。
