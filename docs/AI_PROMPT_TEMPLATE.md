...
这是外部 AI 和兼容导入流程的备用模板。旅图的主要体验是应用内 AI Action Gateway：直接描述目标，由在线 AI 与实时 Provider 完成规划、查证、预览和执行。只有在迁移旧流程、离线准备 JSON 或使用外部工具时，才需要复制下面的提示词。
...
1. 日常规划、找票据、补地点和修复行程优先使用旅图全局 AI。
2. 需要兼容导入时，再让外部 AI 输出 JSON。
3. 在旅图设置页选择“导入 AI 行程包”。
4. 查看预览中的“必须修复”和“建议检查”，确认后执行。
5. 导入后核对没有实时来源的地点、坐标、时间和票据信息。
