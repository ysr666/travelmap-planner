...
- Root directory：`/`
- Environment variables：如果启用云端保存，需要配置 `VITE_SUPABASE_URL` 和 `VITE_SUPABASE_ANON_KEY`；如果启用道路路线，需要配置 `VITE_ROUTING_PROVIDER` 和 `VITE_OPENROUTESERVICE_API_KEY`
...
当前为个人使用型 PWA，优先 local-first。项目可选接入 Supabase 云端保存、OpenRouteService 道路路线和用户自配的 Google Maps 能力，但不包含自建后端、实时云同步、Mapbox token 或任何内置收费服务。
