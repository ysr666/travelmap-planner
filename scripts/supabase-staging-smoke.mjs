...
async function authenticate() {
  ...
  if (accessToken && refreshToken) {
    ...
    if (error) fail('Supabase smoke session token was rejected.')
    return
  }
  ...
  if (email && password) {
    ...
    if (error) fail('Supabase smoke email/password sign-in failed.')
    return
  }
  ...
  if (email) {
    ...
    if (verifyError) fail('Supabase smoke OTP verification failed.')
    return
  }
