...
- Patch apply boundary：AI 修改建议不会自动写库；`update_item` / `move_item` / `delete_item` / `add_item` 只在最终确认后通过本地事务应用，且不触发路线、票据或云端操作。
...
1. Trip Home 构建已脱敏的旅行上下文：trip/day/item IDs、标题、日期、时间、地点文本和交通摘要。
2. 默认不发送 notes；只有 AI Privacy 允许 note summary 时才发送截断摘要。
3. 不发送坐标、票据文件名/内容/blob、cloud token/status、route cache、provider key、URL 或完整本地 DB。
4. 用户通过第一道 ConfirmDialog 确认后，才调用 `ai_trip_edit_plan`。
5. 返回内容必须经过 JSON extraction 和 `validateAiTripEditPatchPlan`。
6. UI 只显示 diff preview；preview 出现不代表已修改旅行。
7. 用户通过第二道 ConfirmDialog 确认后，才在本地 IndexedDB transaction 中应用 patch。
...
- 模型输出未知 operation 时自动猜测或修复。
- 删除已绑定票据的 item。AI 不删除 `ticketMetas` / `ticketBlobs`，也不自动解除票据绑定。
- 写入 route cache、触发 route generation、上传 cloud、创建票据或调用 `travel_search`。
