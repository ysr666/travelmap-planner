...
test('Package 5 票据费用草稿确认后进入 review queue 并持久显示完成记录', async ({ page }) => {
  ...
  await operationsPanel.locator('summary').filter({ hasText: '查看建议' }).click()
  const completed = operationsPanel.locator('summary').filter({ hasText: '完成了什么' })
  await expect(completed).toBeVisible()
  await completed.click()
  await expect(page.getByTestId('trip-operations-history')).toContainText('已从票据生成费用草稿')
  await page.reload({ waitUntil: 'domcontentloaded' })
  await openTripHomeSecondaryTools(page)
  const reloadedOperationsPanel = page.getByTestId('trip-operations-panel')
  await reloadedOperationsPanel.locator('summary').filter({ hasText: '查看建议' }).click()
  await reloadedOperationsPanel.locator('summary').filter({ hasText: '完成了什么' }).click()
  await expect(page.getByTestId('trip-operations-history')).toContainText('已从票据生成费用草稿')
})
