...
import { clearTravelDatabase, expectNoHorizontalOverflow, openDetailsSection, seedTravelRecords } from './helpers'

test('旅行账单档案支持确认接收、时间线、完整性、查询和结算', async ({ page }) => {
  const now = Date.now()
  const trip = {
    createdAt: now,
    destination: '日本东京',
  ...
  await expect(inboxPanel.getByTestId('travel-inbox-expense-suggestions')).toContainText('浅草晚餐订单')
  await expect(inboxPanel.getByTestId('travel-inbox-expense-suggestions')).toContainText('可能是现场消费')
  await inboxPanel.locator('summary').filter({ hasText: '查看建议' }).click()

  const expenseCountBeforeConfirm = await countLedgerExpenses(page, trip.id)
  expect(expenseCountBeforeConfirm).toBe(0)
  await inboxPanel.getByRole('button', { name: '生成草稿' }).click()
  expect(await countLedgerExpenses(page, trip.id)).toBe(0)
  const draftDialog = page.getByRole('dialog', { name: '从旅行材料生成费用草稿？' })
  await expect(draftDialog).toContainText('不会自动计入支出')
  await draftDialog.getByRole('button', { name: '生成草稿' }).click()
  await expect(draftDialog).toBeHidden({ timeout: 15_000 })
  await expect.poll(() => countLedgerExpenses(page, trip.id), { timeout: 15_000 }).toBe(1)

  await page.goto(`/#/ledger?tripId=${trip.id}`, { waitUntil: 'domcontentloaded' })
  await expect(page.getByTestId('ledger-review-queue')).toContainText('浅草晚餐订单')
...
async function countLedgerExpenses(page: import('@playwright/test').Page, tripId: string) {
  return page.evaluate(async (targetTripId) => {
    ...
    const transaction = db.transaction('ledgerExpenses', 'readonly')
    const indexRequest = transaction.objectStore('ledgerExpenses').index('tripId').count(targetTripId)
