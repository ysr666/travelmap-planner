...
test('Trip Home 云端保存冲突提醒展示版本来源并明确原地更新语义', async ({ page }) => {
  ...
  const card = page.getByTestId('trip-home-cloud-save-card').filter({ hasText: '本地和云端可能都有更新' })
...
test('Trip Home 本地版本较新时上传本地数据需要二次确认', async ({ page }) => {
  ...
  const card = page.getByTestId('trip-home-cloud-save-card').filter({ hasText: '本地版本较新' })
