...
test('Trip Home 云端保存冲突提醒展示版本来源并明确原地更新语义', async ({ page }) => {
  ...
  await expect(dialog).toContainText('这是按方向覆盖，不会自动合并本地和云端修改。')
...
test('Trip Home 本地版本较新时上传本地数据需要二次确认', async ({ page }) => {
  ...
  await expect(dialog).toContainText('将用当前本地版本更新云端保存。')
  await expect(dialog).toContainText('云端原有版本会被覆盖。')
  await expect(dialog).toContainText('不会创建新的云端快照列表。')
  await expect(dialog).toContainText('不会自动合并云端修改。')

  await dialog.getByRole('button', { name: '取消' }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await expect(card).toContainText('本地版本较新')
  await expect(page.getByText('本地版本已上传，云端保存已覆盖更新。')).toHaveCount(0)
  await expectNoHorizontalOverflow(page)
})
...
test('设置页云端列表展示历史遗留的同一旅行多条云端保存', async ({ page }) => {
  ...
  await expect(list.getByTestId('cloud-restore-backup').first()).toContainText('用云端覆盖本地')
  await list.getByTestId('cloud-restore-backup').first().click()
  const dialog = page.getByRole('dialog')
  await expect(dialog).toContainText('用云端覆盖本地？')
  await expect(dialog).toContainText('将用云端版本覆盖当前本地旅行。')
  await expect(dialog).toContainText('不会创建新的本地旅行副本。')
