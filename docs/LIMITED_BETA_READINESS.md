...
The largest beta blockers are provider abuse controls and production smoke coverage: quota is still in-memory/dev-only, origin and account/IP controls are not production-grade, and real browser plus Cloudflare Pages/worker smoke is still needed for provider-backed AI and route paths. Real travel search is not ready and should stay disabled or clearly marked as mock/foundation-only.

AI Draft generation and repair are beta-usable only with caveats: they have preview/confirm boundaries and real DeepSeek smoke, but need near-production Pages/worker smoke and durable quota before real users. AI Trip Edit patch plans are safer than direct writes because of sanitized context, diff preview, stale checks, and second confirmation, but should still be treated as foundation-only until full real-provider browser/worker QA passes.
...
| 11. Quota / provider keys / secrets | Blocked | Server-only route/AI keys are designed for env bindings; Settings hides route/AI provider keys; normalized errors avoid raw provider output; Google Maps JS rendering key is separated from server-only Routes key. | Quota guard is in-memory/dev foundation; browser session ID is spoofable; origin allowlist is only env-driven; Google Maps JS key is browser-visible by nature. | Abuse, provider spend, leaked or unrestricted browser key, shared worker memory resets, no account/IP durable enforcement. | Implement durable quota, explicit origin allowlist, account/session/IP controls, budget/alerting, key rotation plan, and Google Maps JS domain restrictions before inviting real users. | Per-account quotas, admin dashboard, safe telemetry. | Codex | `fix/durable-provider-quota` |
...
1. Implement durable provider quota and abuse controls.
   - Replace the in-memory Map quota store with KV, Supabase, Redis, or equivalent.
   - Use account/session/IP/server-observed signals, isolated namespaces for route, AI draft, repair, edit, and search, and production budget alerts.
...
- AI Draft generation/repair, once durable quota and real worker/browser smoke pass, because final import still requires confirmation.
...
- Durable provider quota, abuse dashboard, and billing controls.
...
- Set Cloudflare env: route proxy URL/provider, server-only ORS/Google Routes key, AI provider/base/model/key if enabled, origin allowlist, Supabase vars if cloud is enabled.
...
1. `fix/durable-provider-quota` - durable quota, origin allowlist, account/IP controls, budget alerts.
