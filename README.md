...
- Root directory：`/`
- Environment variables：如果启用云端备份，需要配置 `VITE_SUPABASE_URL` 和 `VITE_SUPABASE_ANON_KEY`
...
当前为个人使用型 PWA，优先 local-first。项目可选接入 Supabase 云端备份，但不包含自建后端、实时云同步、Google Maps API、Mapbox token 或任何需要绑定账单的地图服务。
