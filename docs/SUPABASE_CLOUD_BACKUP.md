...
旅图 TripMap 的第一版云端能力只做“账号登录 + 云端快照备份/恢复”。IndexedDB 仍是主数据源，Supabase 只保存旅行快照和 copy 模式票据附件。

本功能不做实时同步、多设备冲突合并、多人协作或云端编辑。应用支持用户手动上传云端快照，也支持用户在设置页主动开启“自动云端快照备份”，在本地数据变化后延迟创建新的云端快照。
...
- 自动云端快照备份只是可选的快照上传，不会自动恢复、不会覆盖本地，也不会合并多设备修改。
- 恢复云端快照会创建新的本地旅行副本，不会覆盖当前本地旅行，也不会删除云端快照。
- 恢复副本会保存本地展示用来源 metadata；这些字段只用于显示来源，不参与版本比较、dirty 状态、启动提示分组或上传行为。
...
第一版应用不会更新云端快照 metadata，因此不需要 update policy。
...
如果不配置环境变量，应用仍可使用本地 IndexedDB、zip 备份和 AI 行程包导入；SettingsPage 会显示云端快照未配置。

## 自动云端快照备份

设置页可以开启“自动云端快照备份”。开启后，旅图会在本地 Trip / Day / Item / Ticket 数据成功变更后，把对应旅行标记为待备份，并在用户已登录、Supabase 已配置、浏览器在线时延迟上传新的云端快照。
...
- 不会覆盖本地 IndexedDB。
- 不会合并多设备冲突。
- 删除本地旅行不会自动删除云端快照；云端删除仍需要用户手动确认。
- 上传失败不会阻止本地编辑，只会保留待备份状态并显示“云端快照失败，可稍后重试”。
- 本地 zip 备份仍建议保留，尤其是出发前的重要旅行。

## 隐私和安全

- 云端快照会上传旅行数据和 copy 模式票据文件到 Supabase。
...
不是。它是快照备份。用户可以手动上传，也可以主动开启自动云端快照备份，让应用在本地变更后延迟创建新快照；但它不会实时同步、不会自动恢复，也不会自动合并多设备修改。
