...
test('桌面 Beta smoke 覆盖核心页面与 AI 确认边界', async ({ page }) => {
  ...
  await expect(page.getByText('旅行工具', { exact: true })).toBeVisible()
  ...
  await expect(page.getByTestId('item-detail-core')).toBeVisible()
  await expect(page.getByRole('button', { name: '查找地点信息' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '应用与通知' })).toBeVisible()
  ...
  await expect(result).toContainText('只读模拟，不会创建事件、重排记录或同步云端')
