...
旅图可以选择接入 Supabase，用于账号登录后的单旅行云端保存备份和恢复。它不是实时同步：不会合并多设备冲突，也不会多人协作。你可以手动上传本地数据，也可以在设置页主动开启“自动云端保存”，让旅图在本机数据变化后延迟覆盖同一个云端保存。

登录且在线时，旅图会比较本地旅行版本信号与最新云端保存 metadata。云端较新时会提示使用云端版本覆盖本地旅行；本地较新且自动云端保存已开启时会补传本地版本并覆盖云端保存；可能双向修改时会提示用户手动选择用本地覆盖云端或用云端覆盖本地。
...
- 云端保存：上传旅行结构化数据和 copy 模式票据附件到 Supabase，适合换设备或清空浏览器后的恢复。从当前版本开始，一个本地 `trip.id` 对应一个云端保存，重复上传会覆盖该保存。
- 自动云端保存：在本机数据变化后自动补传并覆盖同一个云端保存；可能双向修改时不会静默合并。
- 用云端版本覆盖本地会替换同一 `trip.id` 的本地旅行图，不创建新的旅行副本。
- 旧版多条云端记录和旧版恢复出的本地副本可能仍存在；当前版本不会自动迁移、合并、删除或清理这些历史数据。
- 内部 `snapshot.json` 文件名和相关字段仍为兼容旧数据保留；当前用户界面按一对一“云端保存”呈现。
