...
test('Trip Home 现在建议做什么面板只本地生成并把执行动作保持在确认后', async ({ page }) => {
  ...
  await openPanelDetails(panel, '查看建议')
  ...
  let targetRecommendation = panel.getByTestId('trip-operations-recommendation').first()
  await expect(targetRecommendation).toBeVisible()
  const recommendationId = await targetRecommendation.getAttribute('data-id')
  expect(recommendationId).toBeTruthy()
  const recommendationSelector = `[data-testid="trip-operations-recommendation"][data-id="${recommendationId}"]`

  await targetRecommendation.getByRole('button', { name: /稍后处理/ }).click()
  await expect(panel.locator(recommendationSelector)).toHaveCount(0)
  await page.reload({ waitUntil: 'domcontentloaded' })
  await openTripHomeSecondaryTools(page)
  const reloadedPanel = page.getByTestId('trip-operations-panel')
  await expect(reloadedPanel).toBeVisible()
  await openPanelDetails(reloadedPanel, '查看建议')
  targetRecommendation = reloadedPanel.locator(recommendationSelector)
  await expect(targetRecommendation).toHaveCount(0)
  await expireTripIntelligenceLaterStates(page)
  await page.reload({ waitUntil: 'domcontentloaded' })
  await openTripHomeSecondaryTools(page)
  const expiredPanel = page.getByTestId('trip-operations-panel')
  await openPanelDetails(expiredPanel, '查看建议')
  targetRecommendation = expiredPanel.locator(recommendationSelector)
  await expect(targetRecommendation).toHaveCount(1)

  await targetRecommendation.first().getByRole('button', { name: /忽略建议/ }).click()
  await expect(targetRecommendation).toHaveCount(0)
  await page.reload({ waitUntil: 'domcontentloaded' })
  await openTripHomeSecondaryTools(page)
  const hiddenPanel = page.getByTestId('trip-operations-panel')
  await openPanelDetails(hiddenPanel, '查看建议')
  await openPanelDetails(hiddenPanel, '已隐藏建议')
  await hiddenPanel.getByTestId('trip-operations-hidden-list').getByRole('button', { name: '恢复' }).first().click()
  targetRecommendation = hiddenPanel.locator(recommendationSelector)
  await expect(targetRecommendation).toHaveCount(1)
  await page.reload({ waitUntil: 'domcontentloaded' })
  await openTripHomeSecondaryTools(page)
  const restoredPanel = page.getByTestId('trip-operations-panel')
  await openPanelDetails(restoredPanel, '查看建议')
  targetRecommendation = restoredPanel.locator(recommendationSelector)
  await expect(targetRecommendation).toHaveCount(1)

  await targetRecommendation.first().getByTestId('trip-operations-action').click()
...
test('Day View Trip Live Mode 只做本地计算并提供执行入口', async ({ page }) => {
  ...
  const dayIntelligence = page.getByTestId('day-intelligence-card')
  await expect(dayIntelligence).toBeVisible()
  await expect(dayIntelligence).toContainText('今天要处理')
  await expect(dayIntelligence).toContainText('可能小幅迟到')
  ...
  await openPanelDetails(operationsPanel, '查看建议')
  await openPanelDetails(operationsPanel, '完成了什么')
  await expect(page.getByTestId('trip-operations-history')).toContainText('标记完成')
  await page.getByTestId('trip-operations-history').getByRole('button', { name: '清空历史' }).click()
  await expect(page.locator('summary').filter({ hasText: '完成了什么' })).toHaveCount(0)
  await page.reload({ waitUntil: 'domcontentloaded' })
  await expect(page.locator('summary').filter({ hasText: '完成了什么' })).toHaveCount(0)
  expect(providerProxyRequests).toBe(0)
  await expectNoHorizontalOverflow(page)
...
async function openPanelDetails(panel: Locator, title: string) {
  const summary = panel.locator('summary').filter({ hasText: title }).first()
  await expect(summary).toBeVisible()
  const isOpen = await summary.evaluate((element) => (element.parentElement as HTMLDetailsElement | null)?.open ?? false)
  if (!isOpen) {
    await summary.click()
  }
  ...
  await page.evaluate(async () => {
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        const transaction = database.transaction('tripIntelligenceSuggestionStates', 'readwrite')
        const store = transaction.objectStore('tripIntelligenceSuggestionStates')
        const cursorRequest = store.openCursor()
        cursorRequest.onerror = () => reject(cursorRequest.error)
        cursorRequest.onsuccess = () => {
          const cursor = cursorRequest.result
          if (!cursor) return
          const state = cursor.value as { status?: string; until?: number; zonedDate?: string }
          if (state.status === 'later') {
            cursor.update({ ...state, until: Date.now() - 1, zonedDate: '2026-04-12' })
          }
          cursor.continue()
        }
...
test('Trip Home 旅行收件箱确认后生成 diff 并只在最终确认后写入当前旅行', async ({ page }) => {
  ...
  await expect(panel).toBeVisible()
  await expect(panel).toContainText('旅行材料')
  ...
  await openPanelDetails(operationsPanel, '查看建议')
  await openPanelDetails(operationsPanel, '完成了什么')
  await expect(page.getByTestId('trip-operations-history')).toContainText('已应用旅行材料建议')
  await expect(page.getByTestId('trip-operations-history')).toContainText('创建内容 · 西湖游船票据')
...
test('Trip Home 今日旅行提示本地展示并在确认后保存增强提示', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog')).toContainText('预计查询')
  expect(travelSearchRequests).toBe(0)
  expect(dailyTipRequests).toBe(0)
...
test('Trip Home 地图预览缓存路线并通过 proxy 应用路线顺序建议', async ({ page }) => {
  ...
  await expect(routeDialog).toContainText('生成 2 天路线预览')
  await expect(routeDialog).toContainText('不会自动调整')
  await expect(routeDialog).toContainText('只用已有坐标')
  expect(await readRouteCacheEntryCount(page)).toBe(0)
...
test('Trip Home AI 修改建议需要两次确认且只在最终确认后写入', async ({ page }) => {
  ...
  await expect(panel).toContainText('旅图先给修改预览')
  ...
  await expect(sendDialog).toContainText('应用前会让你确认')
  ...
  await expect(applyDialog).toContainText('写入当前旅行')
...
test('Trip Home 智能整理此行程先确认再生成可勾选 diff 并批量应用', async ({ page }) => {
  ...
  await expect(sendDialog).toContainText('预计查询')
  await expect(sendDialog).toContainText('结果会先给你预览')
...
test('Trip Home 智能整理分阶段失败后可单独重新生成路线顺序', async ({ page }) => {
  ...
  await expect(stageDialog).toContainText('预计查询')
  expect(routeOrderRequests).toBe(1)
...
test('Trip Home 内容补充使用 Places 优先来源并在确认后写入结构化字段', async ({ page }) => {
  ...
  await expect(confirmDialog).toContainText('预计查询')
  expect(placeLookupRequests).toBe(0)
  expect(placeDetailsRequests).toBe(0)
  expect(travelSearchRequests).toBe(0)
  expect(contentEnrichmentRequests).toBe(0)
...
test('Trip Home AI 修改建议搜索意图先确认并显示来源', async ({ page }) => {
  ...
  await panel.getByRole('button', { name: '生成修改方案' }).click()
  const sendDialog = page.getByTestId('ai-trip-edit-send-confirm-dialog')
  await expect(sendDialog).toContainText('并先查询一次相关来源')
  expect(travelSearchRequests).toBe(0)
  expect(editRequests).toBe(0)
  await sendDialog.getByRole('button', { name: '暂不发送' }).click()
  expect(travelSearchRequests).toBe(0)
